...
import com.forkstone.tokens.tweens.VectorAccessor;
...
//      -----------------------------
        ...
        if (Settings.DEBUG) {
            ...
            shapeRenderer.setColor(com.forkstone.tokens.helpers.FlatColors.DARK_GREEN);
    ...
    public void fadeIn(float duration, float delay, float alpha) {
        sprite.setAlpha(0);
        Tween.to(getSprite(), SpriteAccessor.ALPHA, duration).target(alpha).delay(delay)
                .ease(TweenEquations.easeInOutSine).start(manager);
    }
    ...
    public void effectY(float from, float to, float duration, float delay, int repeats, float delayYoyo) {
        position.y = from;
        Tween.to(position, VectorAccessor.VERTICAL, duration).target(to).delay(delay).repeatYoyo(repeats, delayYoyo)
                .ease(TweenEquations.easeInOutSine).start(manager);
    }
    ...
    public void effectX(float from, float to, float duration, float delay, int repeats, float delayYoyo) {
        position.x = from;
        Tween.to(position, VectorAccessor.HORIZONTAL, duration).target(to).delay(delay).repeatYoyo(repeats, delayYoyo)
                .ease(TweenEquations.easeInOutSine).start(manager);
    }
