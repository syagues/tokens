 ...
 */
public class Board45 extends Board {

    private static final String TAG = "Board45";
    ...
    public Board45(final GameWorld world, float x, float y, float width, float height, TextureRegion texture, int rows, int columns, ArrayList<Integer> tokens, int[] staticTokens, final int sameColor) {
                ...
                if(rows == 4 && columns == 5){
                    ...
                    if(tokens.get(i * columns + j) != null){
                        colorButtons.add(new ColorButton(world,
                                ...
                                buttonSize, buttonSize + 10, com.forkstone.tokens.helpers.AssetLoader.colorButton, Color.BLACK, tokens.get(i * columns + j), i * columns + j, isTokenStatic(i * columns + j)));
                    } else {
                    ...
                    if(gameOverTokens.get(i * columns + j) != null){
                        if(gameOverTokens.get(i * columns + j) == -1){
                            colorButtons.add(new ColorButton(world,
                                    tokenPositions.get(i * columns + j).x,
                                    tokenPositions.get(i * columns + j).y,
                                    buttonSize, buttonSize + 10, AssetLoader.colorButton, Color.BLACK, 1, i * columns + j, false));
                            ...
                            colorButtons.add(new ColorButton(world,
                                    tokenPositions.get(i * columns + j).x,
                                    tokenPositions.get(i * columns + j).y,
                                    buttonSize, buttonSize + 10, AssetLoader.colorButton, Color.BLACK, gameOverTokens.get(i * columns + j), i * columns + j, isTokenStatic(i * columns + j)));
                        }
                    } else {
                        gapPosition = i * columns + j;
                    }
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                tokenPositions.add(new Vector2(
                        ...
                        getRectangle().y + (i * buttonSize) + (spaceBetweenTokens * i) + (spaceBetweenTokens) + 123));
            }
        }
    }
        ...
        if(AssetLoader.getDevice().equals(Settings.DEVICE_SMARTPHONE)) {
            ...
            for (Integer movablePosition : world.getLevels().getMovablePositionsArray()) {
                movableCounters.add(new Counter(world,
                        tokenPositions.get(movablePosition).x, tokenPositions.get(movablePosition).y + Settings.BUTTON_SIZE - 45,
                        60, 60, AssetLoader.fontXXS, world.parseColor(Settings.COLOR_WHITE, 1f), "movableCountdown", true, world.getLevelColor(), 13f));
            }
        } else if(AssetLoader.getDevice().equals(Settings.DEVICE_TABLET_1610)) {
            movableCounters.clear();
            for (Integer movablePosition : world.getLevels().getMovablePositionsArray()) {
                movableCounters.add(new Counter(world,
                        tokenPositions.get(movablePosition).x, tokenPositions.get(movablePosition).y + Settings.BUTTON_SIZE - 40,
    ...
    public void checkIfButtonsTouchUp(int screenX, int screenY) {
        ...
        ArrayList<com.forkstone.tokens.ui.MenuButton> menuButtons = world.getMenuButtons();
        for (com.forkstone.tokens.ui.MenuButton menuButton : menuButtons) {
            if(menuButton.isTouchUp(screenX,screenY)){}
        }
        ArrayList<com.forkstone.tokens.ui.MenuButton> gameOverButtons = world.getGameOver().getMenuButtons();
        for (com.forkstone.tokens.ui.MenuButton gameOverButton : gameOverButtons) {
    ...
    public int ableToMove(int buttonNumber) {
        ...
        switch (gapPosition) {
            ...
            case 7:
                ...
                if(buttonNumber == 12) {return 4;}
                break;
            case 8:
                if(buttonNumber == 3) {return 2;}
                if(buttonNumber == 7) {return 3;}
                if(buttonNumber == 9) {return 1;}
                if(buttonNumber == 13) {return 4;}
                break;
            case 9:
                if(buttonNumber == 4) {return 2;}
                if(buttonNumber == 8) {return 3;}
                if(buttonNumber == 14) {return 4;}
                break;
            case 10:
                if(buttonNumber == 5) {return 2;}
                if(buttonNumber == 11) {return 1;}
                if(buttonNumber == 15) {return 4;}
                break;
            case 11:
                if(buttonNumber == 6) {return 2;}
                if(buttonNumber == 10) {return 3;}
                if(buttonNumber == 12) {return 1;}
                if(buttonNumber == 16) {return 4;}
                break;
            case 12:
                if(buttonNumber == 7) {return 2;}
                if(buttonNumber == 11) {return 3;}
                if(buttonNumber == 13) {return 1;}
                if(buttonNumber == 17) {return 4;}
                break;
            case 13:
                if(buttonNumber == 8) {return 2;}
                if(buttonNumber == 12) {return 3;}
                if(buttonNumber == 14) {return 1;}
                if(buttonNumber == 18) {return 4;}
                break;
            case 14:
                if(buttonNumber == 9) {return 2;}
                if(buttonNumber == 13) {return 3;}
                if(buttonNumber == 19) {return 4;}
                break;
            case 15:
                if(buttonNumber == 10) {return 2;}
                if(buttonNumber == 16) {return 1;}
                break;
            case 16:
                if(buttonNumber == 11) {return 2;}
                if(buttonNumber == 15) {return 3;}
                if(buttonNumber == 17) {return 1;}
                break;
            case 17:
                if(buttonNumber == 12) {return 2;}
                if(buttonNumber == 16) {return 3;}
                if(buttonNumber == 18) {return 1;}
                break;
            case 18:
                if(buttonNumber == 13) {return 2;}
                if(buttonNumber == 17) {return 3;}
                if(buttonNumber == 19) {return 1;}
                break;
            case 19:
                if(buttonNumber == 14) {return 2;}
                if(buttonNumber == 18) {return 3;}
                break;
        }
        return 0;
    }
...
//            Gdx.app.log(TAG, "New ColorButton Position: " + getColorButtons().get(colorButtonNumber).getTablePosition());
//            Gdx.app.log(TAG, "GapPosition: " + getGapPositions());
            world.restStep();
            if(world.isMovableLevel()) {
                checkMovableGame();
                checkOkSprites();
    ...
    public void checkOkSpritesFour(){
        ...
        for(ColorButton colorButton : colorButtons) {
            ...
            if(colorButton.getTokenState() == ColorButton.TokenState.INITIAL){
                ...
                // Able to move up
                if(colorButton.getTablePosition() < 15){
                    if(getGapPositions() != colorButton.getTablePosition()+5){
                        if(colorButton.getType() == getColorButtonByTablePosition(colorButton.getTablePosition()+5).getType() ||
                                colorButton.isMulticolor() || getColorButtonByTablePosition(colorButton.getTablePosition()+5).isMulticolor()){
                            coincidences++;
