 ...
 */
public class Board44 extends Board {

    private static final String TAG = "Board44";
    ...
    public Board44(final GameWorld world, float x, float y, float width, float height, TextureRegion texture, int rows, int columns, ArrayList<Integer> tokens, int[] staticTokens, final int sameColor) {
            ...
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < columns; j++) {
                    tokenPositions.add(new Vector2(
                            (j * buttonSize) + ((spaceBetweenTokens - 4) * j) + getRectangle().x + (spaceBetweenTokens) + 4,
                            getRectangle().y + (i * buttonSize) + (spaceBetweenTokens * i) + (spaceBetweenTokens) + 8));
                }
            }
        } else {
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < columns; j++) {
                    tokenPositions.add(new Vector2(
                            ...
                            getRectangle().y + (i * buttonSize) + (spaceBetweenTokens * i) + (spaceBetweenTokens) - 15));
                }
            }
        }
    }
        ...
        if(AssetLoader.getDevice().equals(Settings.DEVICE_SMARTPHONE)) {
            ...
            for (Integer movablePosition : world.getLevels().getMovablePositionsArray()) {
                movableCounters.add(new Counter(world,
                        tokenPositions.get(movablePosition).x, tokenPositions.get(movablePosition).y + Settings.BUTTON_SIZE,
            ...
            for (Integer movablePosition : world.getLevels().getMovablePositionsArray()) {
                movableCounters.add(new Counter(world,
                        tokenPositions.get(movablePosition).x + 5, tokenPositions.get(movablePosition).y + Settings.BUTTON_SIZE - 20,
    ...
    public int ableToMove(int buttonNumber) {
        ...
        switch (gapPosition) {
            ...
            case 14:
                ...
                if(buttonNumber == 15) {return 1;}
                break;
            case 15:
                if(buttonNumber == 11) {return 2;}
                if(buttonNumber == 14) {return 3;}
                break;
        }
        return 0;
    }
    ...
    public void checkOkSpritesFour(){
        ...
        for(ColorButton colorButton : colorButtons) {
            ...
            if(colorButton.getTokenState() == ColorButton.TokenState.INITIAL){
                ...
                if(colorButton.getTablePosition() < 12){
                    if(getGapPositions() != colorButton.getTablePosition()+4){
                        if(colorButton.getType() == getColorButtonByTablePosition(colorButton.getTablePosition()+4).getType()||
                                colorButton.isMulticolor() || getColorButtonByTablePosition(colorButton.getTablePosition()+4).isMulticolor()){
                            coincidences++;
